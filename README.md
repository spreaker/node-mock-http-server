...
| `path`          |                                          | HTTP request path to match. It can have parameters as defined by [`path-to-regexp`](https://www.npmjs.com/package/path-to-regexp). Ex. `/user/:id`. `id` can be accessed later using `req.param('id')` |
